          ...
          >
            <template slot="vehicule_actions" slot-scope="props">
              <div>
                <b-button
                  :id="props['rowData'].id"
                  v-on:click="showVehiculeDialog(props, props['rowData'].id)"
                  variant="outline-primary"
                  >Voir les Vehicules liées à la caserne</b-button
                >
              </div>
            </template>
            <template slot="erase_actions" slot-scope="props">
              <div>
                <b-button
                  :id="props['rowData'].id"
                  v-on:click="showMsgBoxOne(props, props['rowData'].id)"
                  variant="danger"
                  >Supprimer</b-button
                >
              </div>
...
export default {
  ...
  methods: {
    ...
    showMsgBoxOne(props, val) {
      this.boxOne = "";
      this.$bvModal
        .msgBoxConfirm("Etes-vous sûr de vouloir supprimer cet caserne?")
        .then(value => {
          //this.boxOne = value
          if (value === true) {
            console.log("erase");
    ...
    eraseLight(props, val) {
       ...
       this.$store.dispatch("Barrack/deleteBarrack", {token: this.$store.getters["User/token"], barrackId: val}).then((response) => {
    ...
    submit() {
      this.$store.dispatch("Barrack/addBarrack", {token: this.$store.getters["User/token"], city: this.city, name: this.nom, address: this.address}).then((response) => {
           this.$store.dispatch("Barrack/getBarracks", this.$store.getters["User/token"]).then((response) => {
              this.label = this.$store.getters['Barrack/barracks'];
           })
      })
    }
  }
